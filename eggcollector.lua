-- Kyeggo Ground Collector + Rare Detector (Fixed Movement)
-- Uses MoveTo instead of hard teleport
...
local function collectItems()
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        ...
        if itemsFound >= 6 then break end

        if (obj.Name == "Egg" or obj.Name == "Easter Egg") and obj:IsA("MeshPart") then
            local pos = obj.Position
            local dist = (root.Position - pos).Magnitude
            
            if dist < 130 and pos.Y < 60 then  -- Ground only
                itemsFound += 1
                
                -- Walk to egg instead of teleport
                humanoid:MoveTo(pos + Vector3.new(0, 3, 0))
                task.wait(0.6)  -- Give time to walk
                
                -- Small extra wait to make sure it reaches
                ...
                task.wait(0.3) -- Small delay between eggs
            end
        end
    end
end
...
print("✅ Fixed Kyeggo Collector Loaded! (Now using Walk-to method)")
